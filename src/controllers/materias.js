...
const verMaterias = async (req, res, next) => {
    try {
        const { id_usuario,id_inst } = req.params;
        ...
        let query = `
            ...
            m.periodo
            FROM materias m
            JOIN usuarios_instituciones ui ON m.fk_institucion = ui.id_institucion
            WHERE ui.id_usuario = $1
            AND ui.id_institucion = $2
        `;
        const values = [id_usuario,id_inst];
